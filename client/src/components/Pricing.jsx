...
export function Pricing({ onCheckout }) {
  ...
  const [billing, setBilling] = useState(() => (paywall ? 'monthly' : 'monthly'));
  const [offer, setOffer] = useState(() => getIntroOffer());

  useEffect(() => {
    if (paywall) setBilling('monthly');
  }, [paywall]);
    ...
    <section id="pricing" className={`pricing ${paywall ? 'pricing--paywall' : ''}`}>
      <div className="section-wrap">
        <div className="section-head">
          ...
          <h2>{paywall ? 'בחר מסלול והמשך לנתח' : 'מסלול שבועי, חודשי או בודד'}</h2>
          <p>
            {paywall
              ? 'הניתוח החינמי נוצל · 14 יום החזר כספי מלא'
              : 'הניתוח הראשון חינם · אחר כך מ-₪29/שבוע'}
          </p>
        </div>
        ...
        <div className={`pricing__grid pricing__grid--${visiblePlans.length}`}>
          {visiblePlans.map((plan, i) => {
            ...
            const cta = getPlanCta(plan, { freeUsed, paywall, introOffer: offer });
            const isHighlighted =
              (paywall || offer?.active) && plan.id === 'monthly';
            const btnClass =
            ...
            return (
              <Reveal
                key={plan.id}
                delay={i * 90}
                className={`pricing-card ${plan.popular ? 'pricing-card--popular' : ''} ${plan.pro ? 'pricing-card--pro' : ''} ${hasIntroDiscount ? 'pricing-card--offer' : ''} ${isHighlighted ? 'pricing-card--pulse' : ''}`}
              >
                {hasIntroDiscount && (
